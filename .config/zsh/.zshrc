...
workspace() {
  ...
  if [ -z "$1" ]; then
    #wpath=($(find $HOME -maxdepth 2 -iname Code -type d -print | tr '\n' ' '))
    selected=$(find $HOME/Code -mindepth 1 -maxdepth 1 -type d,l -print | fzf)
    [ -z "$selected" ] && return 0
    cd $selected
